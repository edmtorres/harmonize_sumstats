...
def liftover_batch(df, genome_build, col_chr, col_pos):
    """
    ...
    for chrom, pos in zip(df[col_chr], df[col_pos]):
        if len(chrom) == 1:
...
def perform_liftover(gwas_df, genome_build, col_chr, col_pos):
    ...
    gwas_df = (gwas_df.lazy()
                .map_batches(                                       # Applies the 'liftover_batch' Python function to chunks (batches) of the LazyFrame.
                    lambda df: liftover_batch(df, genome_build, col_chr, col_pos),      
